...
public class Loader extends AutomatedSql {
    ...
    public Object getValue(String col, int i, ResultSet rs) throws SQLException {
        ...
        if (type.equals(String.class)) {
            ...
            try {
                ob = type.getDeclaredMethod("valueOf", String.class).invoke(rs.getString(i));
            } catch (IllegalAccessException e) {
                throw new SQLException("Error loading enum: " + type.getName(), e);
            ...
            try {
                if (isPrimitive) {
                    ...
                    for(int counter = 0 ; counter < object.length() ; counter++){
                        Array.set(newArray, counter, object.get(counter));
                    }
                    ob = newArray;
                } else {
                    JSONArray object = new JSONArray(rs.getString(i));
