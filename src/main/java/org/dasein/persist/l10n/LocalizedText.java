...
public class LocalizedText implements CachedItem {
    private static final long serialVersionUID = -6025749948417757531L;

    static public void addTranslation(DaseinPersist persistence, PersistentCache<LocalizedText> factory, String textGroup, String textCode, Locale forLocale, String translation) throws PersistenceException {
        ...
        Transaction xaction = persistence.getTransaction();

        try {
            LocalizedText text = factory.get(localizationCode);
    ...
    static public Collection<LocalizedText> getTranslations(DaseinPersist persistence, PersistentCache<LocalizedText> factory, String textGroup, String textCode) throws PersistenceException {
        SearchTerm[] terms = new SearchTerm[2];
        terms[0] = new SearchTerm("textGroup", textGroup);
        terms[1] = new SearchTerm("textCode", textCode);
        return factory.find(terms);
    }

    static public Collection<LocalizedText> getTranslations(DaseinPersist persistence, String textGroup, String textCode) throws PersistenceException {
        PersistentCache<LocalizedText> factory = persistence.getCache(LocalizedText.class, "localizationCode");
        SearchTerm[] terms = new SearchTerm[2];
        terms[0] = new SearchTerm("textGroup", textGroup);
        terms[1] = new SearchTerm("textCode", textCode);
        return factory.find(terms);
